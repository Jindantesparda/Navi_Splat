...
const registerRenderEvents = (scene: Scene, events: Events) => {
    ...
    events.function('render.image', async (imageSettings: ImageSettings) => {
        ...
        try {
            ...
            const filename = `${removeExtension(selected?.name ?? 'Navi_Splat')}-image.png`;
    ...
    events.function('render.video', (videoSettings: VideoSettings, fileStream: FileSystemWritableFileStream) => {
        const renderImpl = async () => {
            ...
            try {
                ...
                if (!cancelled && !fileStream) {
                    ...
                    downloadFile((output.target as BufferTarget).buffer, `${removeExtension(currentSplats[0]?.name ?? 'navi_splat')}.${fileExtension}`);
                }
        ...
        // codec reclamation.
        if (navigator.locks) {
            return navigator.locks.request('navi-splat-video-render', renderImpl);
        }
        return renderImpl();
    });
};
